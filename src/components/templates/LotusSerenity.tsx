...
export const LotusSerenity: React.FC<Props> = ({ profile }) => {
  const { t } = useLanguage();
  const { personal, education, family, contact } = profile;

  return (
    <div className="w-full h-full bg-gradient-to-br from-emerald-50 via-white to-rose-50 text-emerald-900 font-serif">
      <div className="grid grid-cols-[0.35fr_0.65fr] h-full">
        <div className="p-5 border-r border-emerald-100 space-y-3 bg-gradient-to-b from-emerald-50 to-rose-50">
          <div className="space-y-1">
            ...
            <p className="text-xs uppercase tracking-[0.2em] text-emerald-600">{education.occupation}</p>
          </div>
          <div className="w-full aspect-[3/4] rounded-2xl overflow-hidden border-2 border-emerald-200 shadow-md bg-white">
            {personal.photoUrl ? <img src={personal.photoUrl} alt="Profile" className="w-full h-full object-cover" /> : <PlaceholderImage className="w-full h-full text-emerald-200" />}
          </div>
          <div className="rounded-xl border border-emerald-100 bg-white/70 p-3 space-y-2">
            <p className="text-[10px] uppercase tracking-[0.2em] text-emerald-600 font-semibold">{t('section.astrology')}</p>
            <div className="space-y-1 text-xs text-emerald-800">
              {personal.rashi && <div className="flex justify-between"><span className="text-emerald-600">{t('field.rashi')}</span><span className="font-medium">{personal.rashi}</span></div>}
              {personal.nakshatra && <div className="flex justify-between"><span className="text-emerald-600">{t('field.nakshatra')}</span><span className="font-medium">{personal.nakshatra}</span></div>}
              {personal.gothra && <div className="flex justify-between"><span className="text-emerald-600">{t('field.gothra')}</span><span className="font-medium">{personal.gothra}</span></div>}
              {personal.manglik && <div className="flex justify-between"><span className="text-emerald-600">{t('field.manglik')}</span><span className="font-medium">{personal.manglik}</span></div>}
            </div>
          </div>
          ...
          <div className="space-y-1.5 text-xs">
            ...
            {contact.email && <p className="text-emerald-800 break-words">{contact.email}</p>}
        ...
        <div className="p-8 space-y-8">
          <div className="bg-white/80 border border-emerald-100 rounded-2xl p-6">
            <SectionTitle title={t('section.personal')} lineClass="bg-emerald-200" />
          ...
          <div className="grid grid-cols-2 gap-6">
            <div className="bg-white/80 border border-emerald-100 rounded-2xl p-6 space-y-3">
              <SectionTitle title={t('section.education')} lineClass="bg-emerald-200" />
            ...
            <div className="bg-white/80 border border-emerald-100 rounded-2xl p-6 space-y-3">
              <SectionTitle title={t('field.aboutMe')} lineClass="bg-emerald-200" />
              <p className="text-sm text-emerald-800 leading-relaxed whitespace-pre-line">{education.aboutMe}</p>
            </div>
          </div>
