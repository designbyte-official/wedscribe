  ...
  return (
    <div className="w-full h-full bg-gradient-to-br from-slate-50 via-white to-blue-50 text-slate-900 font-serif relative overflow-hidden">
      ...
      <div className="relative z-10 grid grid-cols-[0.6fr_1.4fr] gap-10 h-full p-12">
        {/* Sidebar */}
        <div className="bg-white/80 backdrop-blur border border-blue-100 shadow-xl rounded-2xl p-6 flex flex-col gap-4">
          <div className="text-center space-y-2">
            ...
            <p className="text-slate-500 text-xs">{education.company}</p>
          </div>
          ...
          <div className="rounded-xl border border-blue-100 bg-blue-50/60 p-4 space-y-2">
            <p className="text-[10px] uppercase tracking-[0.25em] text-blue-700 font-semibold">{t('section.astrology')}</p>
            <div className="grid grid-cols-2 gap-2 text-xs text-slate-700">
              <div className="flex justify-between"><span className="text-blue-600">{t('field.rashi')}</span><span>{personal.rashi}</span></div>
              <div className="flex justify-between"><span className="text-blue-600">{t('field.nakshatra')}</span><span>{personal.nakshatra}</span></div>
              <div className="flex justify-between"><span className="text-blue-600">{t('field.gothra')}</span><span>{personal.gothra}</span></div>
              <div className="flex justify-between"><span className="text-blue-600">{t('field.manglik')}</span><span>{personal.manglik}</span></div>
            </div>
          </div>

          <div className="rounded-xl border border-blue-100 bg-blue-50/60 p-4 space-y-2">
            <p className="text-[10px] uppercase tracking-[0.25em] text-blue-700 font-semibold">{t('section.contact')}</p>
            <div className="space-y-1.5 text-xs text-slate-700">
              <div className="flex items-center gap-2"><Phone size={12} className="text-blue-500" /> {contact.contactNumber}</div>
              <div className="flex items-center gap-2"><Mail size={12} className="text-blue-500" /> {contact.email}</div>
              <div className="flex items-start gap-2"><MapPin size={12} className="text-blue-500 mt-0.5" /> <span className="whitespace-pre-line leading-relaxed">{contact.address}</span></div>
            </div>
          </div>
        </div>
