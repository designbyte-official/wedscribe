  ...
  return (
    <div className="w-full h-full bg-gradient-to-br from-orange-50 via-amber-50 to-white text-amber-900 font-serif">
      <div className="grid grid-cols-[0.36fr_0.64fr] h-full">
        <div className="p-5 border-r border-amber-200 space-y-3 bg-gradient-to-b from-orange-50 to-amber-50">
          <div className="space-y-1">
            <p className="text-[10px] uppercase tracking-[0.3em] text-amber-600 font-semibold">{t('common.marriageBiodata')}</p>
          ...
          <div className="rounded-xl border border-amber-200 bg-white/70 p-3 space-y-2">
            ...
            <div className="space-y-1 text-xs text-amber-800">
              {personal.rashi && <div className="flex justify-between"><span className="text-amber-600">{t('field.rashi')}</span><span className="font-medium">{personal.rashi}</span></div>}
              {personal.nakshatra && <div className="flex justify-between"><span className="text-amber-600">{t('field.nakshatra')}</span><span className="font-medium">{personal.nakshatra}</span></div>}
              {personal.gothra && <div className="flex justify-between"><span className="text-amber-600">{t('field.gothra')}</span><span className="font-medium">{personal.gothra}</span></div>}
              {personal.manglik && <div className="flex justify-between"><span className="text-amber-600">{t('field.manglik')}</span><span className="font-medium">{personal.manglik}</span></div>}
            </div>
          </div>
