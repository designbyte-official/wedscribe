...
export const PastelPeony: React.FC<Props> = ({ profile }) => {
  const { t } = useLanguage();
  const { personal, education, family, contact } = profile;

  return (
    <div className="w-full h-full bg-gradient-to-br from-rose-50 via-white to-pink-50 text-rose-900 font-serif">
      <div className="grid grid-cols-[0.36fr_0.64fr] h-full">
        <div className="p-6 border-r border-rose-100 space-y-4">
          <div className="space-y-1.5">
            <p className="text-[10px] uppercase tracking-[0.28em] text-rose-500 font-semibold">{t('common.marriageBiodata')}</p>
          ...
          <div className="w-full aspect-[3/4] rounded-2xl overflow-hidden border border-rose-100 shadow-sm">
            {personal.photoUrl ? (
              <img src={personal.photoUrl} alt="Profile" className="w-full h-full object-cover" />
            ) : (
              <PlaceholderImage className="w-full h-full text-rose-300" />
            )}
          </div>
          ...
          <div className="space-y-2 text-xs">
            ...
            {contact.email && <p className="break-words text-rose-700">{contact.email}</p>}
